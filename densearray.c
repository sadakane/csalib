...
static int msize=0;
#define mymalloc(p,n,f) {p = malloc((n)*sizeof(*p)); msize += (f)*(n)*sizeof(*p); /* if (f) printf("malloc %d bytes at line %d total %d ptr=%p\n",(n)*sizeof(*p),__LINE__,msize,p);*/  if ((p)==NULL) {printf("not enough memory (%d bytes) in line %d\n",msize,__LINE__); exit(1);};}
...
//  printf("densearray-size:0 %ld\n",size);
  ...
  densearray_make_selecttbl(); // 最初に1回だけでいいので消す?
...
//      s[m] = i;
  ...
  if (opt & SDARRAY_SELECT1) {
    ...
    for (il = 0; il < nl+1; il++) da->lp[il] = 0;
    mymalloc(da->p,nl+1,1);  size += sizeof(*(da->p))*(nl+1);
    for (il = 0; il < nl+1; il++) da->p[il] = 0;
...
//            da->ss[ms*(L/LLL)+is] = s[il*L+is*LLL] - pp;
      ...
      if (r == 0) {
        ...
        for (il = 0; il < ml*L+1; il++) da->sl[il] = 0;
        mymalloc(da->ss,ms*(L/LLL)+1,1);
        for (il = 0; il < ms*(L/LLL)+1; il++) da->ss[il] = 0;
...
static i64 densearray_select0_naive(densearray *da, i64 i)
{
  i64 l,r,m;
  ...
  while (l <= r) {
    m = (l+r)/2;
    if (i <= densearray_rank0(da,m)) r = m-1;  else l = m+1;
  }
  return l;
}

static i64 densearray_select0_by_rank(densearray *da, i64 ith, int c)
{
  bitvec_t x, *buf;
...
#if 0
  if (p != densearray_select0_naive(da, ii)) {
    printf("select0: i = %ld p = %ld naive = %ld\n", 
      ii, p, densearray_select0_naive(da, ii));
    exit(1);
  }
#endif
  return p;
...
//  if (f == 0) return densearray_select0_naive(da,i);
  if (f == 0) return densearray_select0_by_rank(da,i,0);
...
void densearray2_construct(densearray2 *da, i64 n, bitvec_t *buf, int opt)
{
  ...
  ns = (n+DA2_SB-1)/DA2_SB; // SBの数
  nl = (n+DA2_LB-1)/DA2_LB; // LBの数
