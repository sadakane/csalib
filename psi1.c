...
//  printf("read_psi: psi_id = %d L = %d\n",id,l);
  ...
  switch (id2) {
    case ID_DIFF_GAMMA:
      fprintf(stderr, "#psi format = GAMMA L=%d C=%d\n",l,(id>>7));
      sprintf(fpsi,"%s.psi",fname2);
      break;
    case ID_DIFF_GAMMA_RL:
      fprintf(stderr, "#psi format = GAMMA_RL L=%d C=%d\n",l,(id>>7));
      sprintf(fpsi,"%s.pri",fname2);
      break;
    case ID_DIFF_GAMMA_SPARSE:
      fprintf(stderr, "#psi format = GAMMA_SPARSE L=%d C=%d\n",l,(id>>7));
      sprintf(fpsi,"%s.psi",fname2);
      break;
    case ID_DIFF_GAMMA_RL_SPARSE:
      fprintf(stderr, "#psi format = GAMMA_RL_SPARSE L=%d C=%d\n",l,(id>>7));
      sprintf(fpsi,"%s.pri",fname2);
      break;
    case ID_DIFF_GAMMA_RR:
      fprintf(stderr, "#psi format = GAMMA_RR L=%d C=%d\n",l,(id>>7));
      sprintf(fpsi,"%s.pxi",fname2);
      break;
    default:
      fprintf(stderr, "read_csa: ID %d is not supported.\n",id);
      break;
  }
...
// default functions
  csa->BW = csa_BW_by_psi;
  csa->LF = csa_LF_by_psi;
  csa->BW_LF = csa_BW_LF_by_psi;
