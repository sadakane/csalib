...
#if 0
static void setbits(bitvec_t *B, i64 i, int d, i64 x)
{
  int j;
...
#else
static int setbits(bitvec_t *B, ulong i, int d, ulong x)
{
  ulong y,m;
  int d2;
...
static bitvec_t getbitsCK(bitvec_t *B, i64 i, i64 d, size_t size)
{
  bitvec_t x,z;
  bitvec_t *const base = B;
//  bitvec_t x0;
...
//  x0 = getbits0(B,i,d);
  ...
  if (i+d <= D) {
    ...
    x >>= D-d;
//    if (B + 1 - base >= size) {
//      fprintf(stderr, "range error %lld", size);
//      return x;
//    }
//    z = B[1] >> (D-(i+d-D));
//    x += z;
...
//  x = getbits(in,i,32)<<32;
  x = getbitsCK(in,i,32, da->bufsize)<<32;
#if USE_HUFTBL
...
//    da->rl[i/RR] = r;
  ...
  if (opt & SDARRAY_COMPRANK) {
    ...
    if (opt & SDARRAY_SELECT1) opt2 |= SDARRAY_RANK1;
    sparsearray_construct_end(da->rsa, opt2);
  }
    ...
    if (pass == 1) {
      if (opt & SDARRAY_LENHUF) {
        ...
        for (i=0; i<D+1; i++) {
         ...
         if (da->h->clen[i]>32) printf("%ld: len = %d code = %lx\n",i,da->h->clen[i],da->h->code[i]);
        }
      }
      bs = (bs+D-1)/D;
      mymalloc(da->buf,bs);
      for (i=0; i<bs; i++) da->buf[i] = 0; // 初期化しないとvalgrindでエラーになる
//      printf("buf %p n = %ld\n", da->buf, n);
      da->bufsize = bs;
...
int comparray_getbit(comparray *da, i64 i)
{
  int c;
  i64 r;
  r = comparray_rank_and_bit(da, i, &c);
  return c;
}
#endif
...
i64 comparray_rank_and_bit(comparray *da, i64 i, int *c)
{
  ...
  if (i < 0 || i >= da->n) {
    printf("comparray_rank_and_bit: i = %ld n = %ld\n", i, da->n);
    exit(1);
  }

  if (da->opt & SDARRAY_COMPRANK) { // ブロックごとのランク値が圧縮してある場合
    r = sparsearray_select(da->rsa, i/da->L+1);
  } else { // ブロックごとのランク値が圧縮されずに配列に入っている場合
//  r = da->rl[i>>logRR] + da->rs[i>>logRRR];
    r = getuint(da->rl,i>>logRR,da->k2); // 大ブロックのランク
    r += da->rs[i/da->L]; // 小ブロックのランク
  }
  rr = 0;
  
  buf = da->buf;
//  ofs = da->pl[i>>logRR];
  if (da->opt & SDARRAY_COMPPTR) { // ブロックへのポインタが圧縮されている場合
    ofs = sparsearray_select(da->psa, i/da->L +1);
  } else { // ブロックへのポインタが圧縮されずに配列に入っている場合
    ofs = getuint(da->pl,i>>logRR, da->k1); // 大ブロックへのポインタ
    ofs += da->ps[i/da->L]; // 小ブロックへのポインタ
  }
  i &= da->L -1; // 小ブロック内の位置
  
  if (da->opt & SDARRAY_RR) { // 0,1の連続する個数で符号化されている場合
    d = getbit(buf,ofs); // 最初が0,1のどちらで始まるか
    ofs++;
    if (d == 1) runlen = 1;  else runlen = 0;
    k = 0; // これから読むビットの位置
    *c = 0;
    while (k <= i) {
      if (runlen == 1) {
        ofs += decodegamma(buf,ofs,&d); // 1の連続が何個続くかを表す符号を復号 (答えは d )
        if (k+d-1 > i) { // i が1の範囲の中に入っているなら
          r += i-k+1; // k から i までは全て 1 なのでその分のランクを加える
          *c = 1; // i の場所のビットは 1
          break;
        }
        // i の位置までまだ復号していない
      ...
      ofs += decodegamma(buf,ofs,&d); // 0の連続が何個続くかを表す符号を復号 (答えは d )
      k += d; // 位置を d すすめる
      runlen = 1; // 次は 1 が連続している
    }
  } else if (da->opt & SDARRAY_SUC) {
    i64 blk,b;
    i64 k,w,k2,w2,j2,nn,l,m;
...
i64 comparray_select(comparray *da, i64 ith, int c)
{
  bitvec_t x, *buf;
  ...
  if ((da->opt & SDARRAY_SELECT1) == 0) {
    printf("comparray_select: select is not supported.\n");
  }

  if (ith <= 0 || ith > da->m) {
    printf("comparray_select: i = %ld  m = %ld\n", ith, da->m);
    exit(1);
  }

  if (da->opt & SDARRAY_COMPRANK) { // ブロックごとのランク値が圧縮してある場合
    pl = sparsearray_rank(da->rsa, ith-1)-1;
    if (pl < 0) {
      printf("??? pl = %ld\n", pl);
    }
    if (pl >= 0) {
      r2 = sparsearray_select(da->rsa, pl+1);
      ith -= r2;
    }
  } else { // ブロックごとのランク値が圧縮されずに配列に入っている場合
    ...
    ith -= r2; // 大ブロック内のランク
    ll = (pl<<logRR)/da->L;
    rl = min(((pl+1)<<logRR)/da->L-1, (da->n-1)/da->L);
    pl = ll;  r2 = 0;
    while (ll <= rl) {
      ml = (ll+rl) >> 1;
      r = da->rs[ml];
      if (c == 0) {r = (ml*da->L) - r;} // select0 の場合
      if (r < ith) {
        pl = ml;
        ll = ml+1;
        r2 = r;
  ...
  i = pl * da->L; // ith を含む小ブロックの最初のビット位置
  rr = 0;
  ...
  if (da->opt & SDARRAY_RR) { // 0,1の連続する個数で符号化されている場合
    printf("RR not supported\n");
    exit(1);
  } else if (da->opt & SDARRAY_SUC) {
    printf("SUC not supported\n");
    exit(1);
  } else {
    if (da->opt & SDARRAY_LENHUF) {
      while (1) {
        ofs2 = skip_enum2(da, ofs, D, &rr);
        if (c == 0) {rr = D - rr;} // select0 の場合
        if (rr >= ith) break;
        ofs += ofs2;
        ith -= rr;
        i += D;
...
// comparray_construct(&s2,n,B, 512, SDARRAY_RANK1 | SDARRAY_LENHUF);
 comparray_construct(&s2,n,B, 512, SDARRAY_RANK1 | SDARRAY_SELECT1 | SDARRAY_LENHUF | SDARRAY_COMPRANK*0 | SDARRAY_COMPPTR*0);
...
#endif
#ifdef CHECK
    if (comparray_select(&s1,j,1) != S[j] || 0) {
