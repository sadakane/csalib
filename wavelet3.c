...
static i64 wt3_rank_access(wavelet3 *wt, i64 i, int *cc)
{
  int node;
  int c,n,m;
  i64 r;
  ...
  da = (densearray3 **)wt->da;
  n = huf->n;
  m = huf->m;
  node = n+m - 2;

  while (node >= n && i >= 0) {
    r = densearray3_rank_and_bit(da[node-n], i, &c);
    if (c == 0) {
      i = (i+1) - r - 1;
      node = huf->left[node];
    } else {
...
static int wt3_access(wavelet3 *wt, i64 i)
{
  int c;

  wt3_rank_access(wt, i, &c);
  return c;
}


static i64 wt3_rank(wavelet3 *wt, i64 i, int c)
{
  int node;
  u64 x;
  int d,n,m;
...
//    printf("wt_rank: no c = %d, i = %ld\n", c, i);
  ...
  while (node >= n && i >= 0) {
    d = (x >> (sizeof(x)*8-1)) & 1;
    if (d == 0) {
      i = densearray3_rank0(da[node-n], i) - 1;
      node = huf->left[node];
    } else {
      i = densearray3_rank(da[node-n], i) - 1;
      node = huf->right[node];
    }
    x <<= 1;
  }
...
static i64 wt3_select_naive(wavelet3 *wt, i64 i, int c)
{
  i64 l,r,m;
...
//  if (l != wt3_select_fast(wt,i,c)) {
//    printf("select(i=%ld, c=%d) naive = %ld fast = %ld\n", i, c, l, wt3_select_fast(wt,i,c));
//  }
  return l;
}


static i64 wt3_select_fast(wavelet3 *wt, i64 i, int c)
{
  int node;
  u64 x;
  int d,n,m;
...
//    i = densearray_select(da[node-n], i+1, bit[depth]);
    i = densearray3_select(da[node2[depth]-n], i+1, bit[depth]);
    depth--;
  }
#if 0
  if (i != wt3_select_naive(wt, ii, c)) {
    printf("wt3_select_fast: i = %ld ans = %ld naive = %ld\n",
      ii, i, wt3_select_naive(wt, ii, c));
    exit(1);
  }
#endif
  return i;
...
static void wt3_enum_sub(wavelet3 *wt, i64 l, i64 r, int node, int *num, int *C)
{
  int n;
...
//    putuint(C, *num, node, wt->k);
  ...
  rank1 = densearray3_rank(da[node-n], r);
  if (l > 0) rank2 = densearray3_rank(da[node-n], l-1); else rank2 = 0;
  rank = rank1 - rank2;

  if (rank < r-l+1) { // there exists at least one 0
    wt3_enum_sub(wt, l-rank2, r-rank1, huf->left[node], num, C);
  }
  if (rank > 0) { // there exists at least one 1
    wt3_enum_sub(wt, rank2, rank1-1, huf->right[node], num, C);
  }
}

static int wt3_enum(wavelet3 *wt, i64 l, i64 r, int *C, i64 *freq)
{
  int node;
  int i,c,n,m, num;
  int *Ctmp;
  ...
  num = 0;
  wt3_enum_sub(wt, l, r, node, &num, Ctmp);
...
static void make_wavelet3_sub(wavelet3 *wt, i64 n, int k2, int depth, int node)
{
  i64 i,nl,nr;
  int c,d,m,w;
  FILE *in,*fl,*fr;
  ...
  da[node-m] = mymalloc(sizeof(densearray3));
  densearray3_construct_init(da[node-m], n);
  ...
  for (i=0; i<n; i++) {
    ...
    x = h->code[c];
    d = (x >> (w-1-depth)) & 1;
    densearray3_construct_set(da[node-m], i, d);
    if (d == 0) {
      writeint(k2, c, fl);
      nl++;
    } else {
...
//  printf("nl = %ld nr = %ld\n", nl, nr);
  ...
  stmp = densearray3_construct_end(da[node-m], wt->L, wt->opt);
  printf("densearray3 size=%ld (%1.3f bpc)\n",stmp,(double)stmp*8/n);
  fflush(stdout);
  wt->size += stmp;
  make_wavelet3_sub(wt, nl, k2, depth+1, h->left[node]);
  make_wavelet3_sub(wt, nr, k2, depth+1, h->right[node]);
}

static wavelet3 *make_wavelet3(int k2, int sigma, char *fwt, int opt)
{
  FILE *in,*out;
  i64 i,n;
  int root;
...
//    fputc(csa->CtoA[c],out);
  ...
  wt->da = da = mymalloc(sizeof(densearray3)*sigma);
  for (i=0; i<sigma; i++) da[i] = NULL;

  wt->size = 0;
  make_wavelet3_sub(wt, n, k2, 0, root);
...
i64 wt3_makeindex(i64 k2, int sigma, char *fname, int opt)
{
  ...
  densearray3_make_selecttbl();

  wt = make_wavelet3(k2, sigma, fname, opt);
  huf = (Huffman2 *)wt->huffman;
  da = (densearray3 **)wt->da;
  k = wt->k;
...
//  size += 1;
  ...
  for (i=nn; i<=2*nn-2; i++) {
    if (huf->left[i] < 2*nn) {
      size += densearray3_write(da[i-nn], out);
    }
  }
#else
  nn = huf->m;
  for (i=0; i<nn-1; i++) {
    size += densearray3_write(da[i], out);
  }
#endif
...
void wt3_read(wavelet3 *wt, char *fname)
{
  ...
  if (id != ID_WAVELET3) {
    printf("wt3_read: id = %ld is not supported.\n",id);
    exit(1);
  }
...
//  wt->id = id;
  ...
  for (i=0; i<nn-1; i++) {
    da[i] = mymalloc(sizeof(densearray3));
    densearray3_read(da[i], &p);
  }
  wt->size = p - q;

  wt->access = (int (*)(wavelet3 *, i64))wt3_access;
  wt->rank = (i64 (*)(wavelet3 *, i64, int))wt3_rank;
  wt->rank_access = (i64 (*)(wavelet3 *, i64, int *))wt3_rank_access;
//  wt->select = (i64 (*)(wavelet3 *, i64, int))wt3_select_naive;
  wt->select = (i64 (*)(wavelet3 *, i64, int))wt3_select_fast;
  wt->enumerate = (int (*)(wavelet3 *, i64, i64, int *, i64 *))wt3_enum;
...
int main(int argc, char *argv[])
{
  ...
  wt3_makeindex(k2, sigma, argv[2], opt);
