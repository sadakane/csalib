...
//  lf->huffman = MakeHuffmanTree(m, freq);
  lf->huffman = MakeHuffmanTree2(m, freq);
...
static void make_wavelet_sub(int k2, lf_wt *lf, i64 n, int depth, int node)
{
  i64 i,nl,nr;
  int c,d,m,w;
  FILE *in,*fl,*fr;
...
//      fputc(c, fr);
  ...
  lf->psize += stmp;
//  lf->psize += (n+DD-1)/8;
  make_wavelet_sub(k2, lf, nl, depth+1, h->left[node]);
  make_wavelet_sub(k2, lf, nr, depth+1, h->right[node]);
}
...
//    fputc(csa->CtoA[c],out);
    c = readuint(k2, in);
    writeint(k2, csa->CtoA[c],out);
  }
  fclose(out);
  
  lf->da = mymalloc(sizeof(*lf->da)*csa->sigma);
  for (i=0; i<csa->sigma; i++) lf->da[i] = NULL;

  lf->psize = 0;
  make_wavelet_sub(k2, lf, n, 0, root);
...
//    head[*num] = csa->AtoC[node];
    putuint(head, *num, csa->AtoC[node], csa->k2);
...
int lf_wt_child_l(CSA *csa, i64 l, i64 r, uchar *head, rank_t *ll, rank_t *rr)
{
  ...
  headtmp = mymalloc(m * sizeof(*headtmp) * csa->k2);
  ltmp = mymalloc(m * sizeof(*ltmp));
  rtmp = mymalloc(m * sizeof(*ltmp));
  hc = mymalloc(csa->sigma * sizeof(*hc));

  num = 0;
  lf_wt_child_l_sub(csa, l, r, node, &num, headtmp, ltmp, rtmp);

  for (c=0; c<csa->sigma; c++) hc[c] = -1;
//  for (i=0; i<num; i++) hc[headtmp[i]] = i;
  for (i=0; i<num; i++) hc[getuint(headtmp, i, csa->k2)] = i;
  i = 0;
  for (c=0; c<csa->sigma; c++) {
    if (hc[c] >= 0) {
//      head[i] = headtmp[hc[c]];
      putuint(head, i, getuint(headtmp, hc[c], csa->k2), csa->k2);
      ll[i] = ltmp[hc[c]];
      rr[i] = rtmp[hc[c]];
      i++;
    }
...
//  comparray_maketbl();
  ...
  out = fopen(fidx,"w");
  if (out == NULL) {
    printf("lf_wt_makeindex: cannot open %s\n",fidx);
  }
...
//  printf("lf_wt_read: lf_id = %ld\n",id);
  switch (id) {
    case ID_BWT_WT:
      fprintf(stderr, "#lf format = BWT_WT\n");
      break;
    case ID_BWT_WT_HUF:
      fprintf(stderr, "#lf format = BWT_WT_HUF\n");
      break;
    case ID_BWT_WT_DENSE:
      fprintf(stderr, "#lf format = BWT_WT_DENSE\n");
      break;
    case ID_BWT_WT_SPARSE4:
      fprintf(stderr, "#lf format = BWT_WT_SPARSE4\n");
      break;
    case ID_BWT_WT_RR:
      fprintf(stderr, "#lf format = BWT_WT_RR\n");
      break;
    default:
      fprintf(stderr, "lf_wt_read: ID %ld is not supported.\n",id);
      break;
  }
