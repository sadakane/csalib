...
static i64 wt_rank_access(wavelet *wt, i64 i, int *cc)
{
  int node;
  int c,n,m;
  i64 r;
  ...
  while (node >= n && i >= 0) {
    r = comparray_rank_and_bit(da[node-n], i, &c);
    if (c == 0) {
      i = (i+1) - r - 1;
      node = huf->left[node];
    } else {
...
static int wt_access(wavelet *wt, i64 i)
{
  int c;

  wt_rank_access(wt, i, &c);
  return c;
}


static i64 wt_rank(wavelet *wt, i64 i, int c)
{
  int node;
  u64 x;
  int d,n,m;
...
//    printf("wt_rank: no c = %d, i = %ld\n", c, i);
  ...
  while (node >= n && i >= 0) {
    d = (x >> (sizeof(x)*8-1)) & 1;
    if (d == 0) {
      i = comparray_rank0(da[node-n], i) - 1;
      node = huf->left[node];
    } else {
      i = comparray_rank(da[node-n], i) - 1;
      node = huf->right[node];
    }
    x <<= 1;
  }
...
static i64 wt_select_fast(wavelet *wt, i64 i, int c)
{
  int node;
  u64 x;
  int d,n,m;
...
//    printf("wt_select: no c = %d, i = %ld\n", c, i);
  ...
  i--;
  while (depth >= 0) {
    i = comparray_select(da[node-n], i+1, bit[depth]);
    depth--;
  }
  return i;
}
...
static i64 wt_select_naive(wavelet *wt, i64 i, int c)
{
  i64 l,r,m;
  ...
  if (l != wt_select_fast(wt,i,c)) {
    printf("select(i=%ld, c=%d) naive = %ld fast = %ld\n", i, c, l, wt_select_fast(wt,i,c));
  }
  return l;
}


static void wt_enum_sub(wavelet *wt, i64 l, i64 r, int node, int *num, int *C)
{
  int n;
...
//    putuint(C, *num, node, wt->k);
  ...
  rank1 = comparray_rank(da[node-n], r);
  if (l > 0) rank2 = comparray_rank(da[node-n], l-1); else rank2 = 0;
  rank = rank1 - rank2;

  if (rank < r-l+1) { // there exists at least one 0
    wt_enum_sub(wt, l-rank2, r-rank1, huf->left[node], num, C);
  }
  if (rank > 0) { // there exists at least one 1
    wt_enum_sub(wt, rank2, rank1-1, huf->right[node], num, C);
  }
}

static int wt_enum(wavelet *wt, i64 l, i64 r, int *C, i64 *freq)
{
  int node;
  int i,c,n,m, num;
  int *Ctmp;
  ...
  num = 0;
  wt_enum_sub(wt, l, r, node, &num, Ctmp);
...
static void make_wavelet_sub(wavelet *wt, i64 n, int k2, int depth, int node)
{
  i64 i,nl,nr;
  int c,d,m,w;
  FILE *in,*fl,*fr;
  ...
  da[node-m] = mymalloc(sizeof(comparray));
  comparray_construct_init(da[node-m], n);
  ...
  for (i=0; i<n; i++) {
    ...
    x = h->code[c];
    d = (x >> (w-1-depth)) & 1;
    comparray_construct_set(da[node-m], i, d);
    if (d == 0) {
      writeint(k2, c, fl);
      nl++;
    } else {
...
//  printf("nl = %ld nr = %ld\n", nl, nr);
  ...
  stmp = comparray_construct_end(da[node-m], wt->L, wt->opt);
  printf("comparray size=%ld (%1.3f bpc)\n",stmp,(double)stmp*8/n);
  fflush(stdout);
  wt->size += stmp;
  make_wavelet_sub(wt, nl, k2, depth+1, h->left[node]);
  make_wavelet_sub(wt, nr, k2, depth+1, h->right[node]);
}

static wavelet *make_wavelet(int k2, int sigma, char *fwt, int opt)
{
  FILE *in,*out;
  i64 i,n;
  int root;
...
//    fputc(csa->CtoA[c],out);
  ...
  wt->da = da = mymalloc(sizeof(comparray)*sigma);
  for (i=0; i<sigma; i++) da[i] = NULL;

  wt->size = 0;
  make_wavelet_sub(wt, n, k2, 0, root);
...
i64 wt_makeindex(i64 k2, int sigma, char *fname, int opt)
{
  ...
  wt = make_wavelet(k2, sigma, fname, opt);
  huf = (Huffman2 *)wt->huffman;
  da = (comparray **)wt->da;
  k = wt->k;
...
//  size += 1;
  ...
  for (i=nn; i<=2*nn-2; i++) {
    if (huf->left[i] < 2*nn) {
      size += comparray_write(da[i-nn], out);
    }
  }
#else
  nn = huf->m;
  for (i=0; i<nn-1; i++) {
    size += comparray_write(da[i], out);
  }
#endif
...
void wt_read(wavelet *wt, char *fname)
{
  ...
  if (id != ID_WAVELET) {
    printf("wt_read: id = %ld is not supported.\n",id);
    exit(1);
  }
...
//  wt->id = id;
  ...
  for (i=0; i<nn-1; i++) {
    da[i] = mymalloc(sizeof(comparray));
    comparray_read(da[i], &p);
  }
  wt->size = p - q;

  wt->access = (int (*)(wavelet *, i64))wt_access;
  wt->rank = (i64 (*)(wavelet *, i64, int))wt_rank;
  wt->rank_access = (i64 (*)(wavelet *, i64, int *))wt_rank_access;
//  wt->select = (i64 (*)(wavelet *, i64, int))wt_select_naive;
  wt->select = (i64 (*)(wavelet *, i64, int))wt_select_fast;
  wt->enumerate = (int (*)(wavelet *, i64, i64, int *, i64 *))wt_enum;
...
int main(int argc, char *argv[])
{
  ...
  wt_makeindex(k2, sigma, argv[2], opt);
